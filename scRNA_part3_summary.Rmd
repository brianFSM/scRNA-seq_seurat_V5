...
metrics.table.string <- p1$generate_metrics_tables
run.doubletFinder <- as.logical(p2$run_doubletFinder)
run.soupx <- as.logical(p1$run_SoupX)
...
################################################################################
analyst.name <- cfg$project$`analyst-name`
formatted.samples <- cfg$project$samples # Sanitized below
experiment.name = cfg$project$`project-name` 
project.description <- cfg$project$`project-decription`
dataset.path <- cfg$data$parent_directory_path
import.h5 <- cfg$data$`import-h5` # set to false if you want to import the matrix files
organism = cfg$project$organism
num.clusters <- p2$num_clusters
...
pct.y.path <- file.path(ggplot.directory,  "percent_y_distribution.png")

knitr::include_graphics(pct.y.path)
...
```{r soupxMsg, eval=run.soupx, results='asis'}
cat("### SoupX\n\nSoupX was run on each sample individually upon import to remove ambient RNA.\n")
```
